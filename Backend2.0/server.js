...
app.post("/api/payment/initiate", async (req, res) => {
  ...
  console.log(
    `✅ PhonePe Payment initiated for ${email}, TXN: ${merchantTransactionId}`,
  );
  try {
...
app.post("/api/payment/initiate-change-seat", async (req, res) => {
  ...
  if (realConflicts > 0) {
    return res.status(400).json({
      message: "Selected seat is not available for the entire duration.",
    });
  }
...
app.post("/api/book-cash", async (req, res) => {
  ...
  if (conflicts.length > 0) {
    return res
      .status(400)
      .json({ message: "Seat already booked during this period.", conflicts });
  }
...
app.get("/api/payment/status", async (req, res) => {
  ...
  if (!txnId) {
    return res
      .status(400)
      .json({ code: "MISSING_TXN_ID", message: "Missing transaction ID" });
  }
